...
jmemviz はいまのところ、[Java Object Layout (JOL)](https://github.com/openjdk/jol)
を使って **JVM ヒープ上のオブジェクトのメモリ上の配置** を観察する、教育目的のツールです。

**※注意**:
正確には、現時点では jmemviz は上に書いたようなことができるといいなぁ、
と思って作った、モックアップに毛の生えたようなものです。
正確性や有用性はまだ検討途上のものです。
...
そこで、学生が使う機会のある言語で、同種の観察ができるようなツールが欲しいと思い、
jmemviz を作成しました。

jmemviz は Java を対象にしています。Java を選んだ理由は、学生が使ったことがある、
あるいは今後使う機会がありそうな言語であること、
また、オブジェクトレイアウトが比較的辿りやすいことが理由です。

現在の実装では、ごく基本的なプリミティブ型、クラス、配列のオブジェクトレイアウトをサポートしています。
メモリ上にオブジェクトがどのように配置され、それが変化していくのかをデモンストレーションする、
という目的ではまぁまぁ十分に動いています。

### jmemviz は何であり、何ではないか

jmemviz はあくまでも教育用途での使用が前提です。
その使い方は、教師が学生に対して、あらかじめ用意したプログラムを用いてデモンストレーションする、
というのが基本形です。

Java で扱えるすべてのオブジェクトをサポートする予定はありませんし、
すべての構文をサポートすることもまた興味の範囲外です。

Java で書かれたどんなプログラムでもステップ実行してオブジェクトレイアウトを確かめられるような万能ツールを目指すつもりはありません。
デモのために、プログラムに手を加え、プリプロセッサを実行する必要もあります。デバッグ支援機能を使えばもうちょっとかっこいい実装はできるかもしれませんが、
優先順位は高くありません。

一方で、プログラミング初心者を対象とするツールでもありません。
コンピュータおよびプログラムの動作について深く理解したい、という層を相手とすることを前提としています。
